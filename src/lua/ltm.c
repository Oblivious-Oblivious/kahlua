...
void luaT_callTMres(
  lua_State *L, const TValue *f, const TValue *p1, const TValue *p2, StkId res
) {
...
void luaT_trybiniTM(
  lua_State *L, const TValue *p1, lua_Integer i2, int flip, StkId res, TMS event
) {
  TValue aux;
  setivalue(&aux, i2);
...
int luaT_callorderiTM(
  lua_State *L, const TValue *p1, int v2, int flip, int isfloat, TMS event
) {
  TValue aux;
  const TValue *p2;
  if(isfloat) {
...
void luaT_adjustvarargs(
  lua_State *L, int nfixparams, CallInfo *ci, const Proto *p
) {
  int i;
