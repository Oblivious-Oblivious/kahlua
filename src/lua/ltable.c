...
*/
static unsigned int
findindex(lua_State *L, Table *t, TValue *key, unsigned int asize) {
  unsigned int i;
  if(ttisnil(key)) {
...
*/
void luaH_resize(
  lua_State *L, Table *t, unsigned int newasize, unsigned int nhsize
) {
  unsigned int i;
...
*/
void luaH_finishset(
  lua_State *L, Table *t, const TValue *key, const TValue *slot, TValue *value
) {
  if(isabstkey(slot)) {
...
static unsigned int
binsearch(const TValue *array, unsigned int i, unsigned int j) {
...
lua_Unsigned luaH_getn(Table *t) {
  ...
  lua_assert(
    limit == luaH_realasize(t) && (limit == 0 || !isempty(&t->array[limit - 1]))
  );
