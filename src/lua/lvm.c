...
*/
void luaV_finishget(
  lua_State *L, const TValue *t, TValue *key, StkId val, const TValue *slot
) {
...
*/
void luaV_finishset(
  lua_State *L, const TValue *t, TValue *key, TValue *val, const TValue *slot
) {
  int loop; /* counter to avoid infinite loops */
  for(loop = 0; loop < MAXTAGLOOP; loop++) {
...
int luaV_equalobj(lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  if(ttypetag(t1) != ttypetag(t2)) { /* not the same variant? */
    if(ttype(t1) != ttype(t2) || ttype(t1) != LUA_TNUMBER) {
      ...
      lua_Integer i1, i2;
      return (
        luaV_tointegerns(t1, &i1, F2Ieq) && luaV_tointegerns(t2, &i2, F2Ieq) &&
        i1 == i2
      );
    }
  }
...
void luaV_finishOp(lua_State *L) {
  ...
  default: {
    ...
    lua_assert(
      op == OP_TFORCALL || op == OP_CALL || op == OP_TAILCALL ||
  ...
  for(;;) {
    ...
    vmdispatch(GET_OPCODE(i)) {
      ...
      vmcase(OP_TAILCALL) {
        ...
        if((n = luaD_pretailcall(L, ci, ra, b, delta)) <
           0) {                   /* Lua function? */
