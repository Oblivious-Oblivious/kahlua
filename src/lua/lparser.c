...
static l_noret errorlimit(FuncState *fs, int limit, const char *what) {
  ...
  msg =
    luaO_pushfstring(L, "too many %s (limit is %d) in %s", what, limit, where);
...
static void check_match(LexState *ls, int what, int who, int where) {
  if(luai_unlikely(!testnext(ls, what))) {
...
static int new_localvar(LexState *ls, TString *name) {
  ...
  Vardesc *var;
  checklimit(
    fs, dyd->actvar.n + 1 - fs->firstlocal, MAXVARS, "local variables"
  );
  luaM_growvector(
    L,
    dyd->actvar.arr,
...
static void solvegoto(LexState *ls, int g, Labeldesc *label) {
  ...
  if(luai_unlikely(gt->nactvar < label->nactvar)) { /* enter some scope? */
...
*/
static int
newlabelentry(LexState *ls, Labellist *l, TString *name, int line, int pc) {
  int n = l->n;
  luaM_growvector(
...
static void close_func(LexState *ls) {
  ...
  luaM_shrinkvector(
    L, f->abslineinfo, f->sizeabslineinfo, fs->nabslineinfo, AbsLineInfo
  );
...
static void checkrepeated(LexState *ls, TString *name) {
  Labeldesc *lb = findlabel(ls, name);
  if(luai_unlikely(lb != NULL)) { /* already defined? */
...
static void fixforjump(FuncState *fs, int pc, int dest, int back) {
  ...
  if(luai_unlikely(offset > MAXARG_Bx)) {
...
static int getlocalattribute(LexState *ls) {
  ...
  if(testnext(ls, '<')) {
    ...
    if(strcmp(attr, "const") == 0) {
      ...
      luaK_semerror(
        ls, luaO_pushfstring(ls->L, "unknown attribute '%s'", attr)
      );
    }
  }
