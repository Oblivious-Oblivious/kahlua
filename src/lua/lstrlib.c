...
static int str_rep(lua_State *L) {
  ...
  if(n <= 0) {
    lua_pushliteral(L, "");
  } else if(luai_unlikely(l + lsep < l || l + lsep > MAXSIZE / n)) {
...
static int str_byte(lua_State *L) {
  ...
  if(luai_unlikely(pose - posi >= (size_t)INT_MAX)) { /* arithmetic overflow? */
...
static int str_dump(lua_State *L) {
  ...
  state.init = 0;
  if(luai_unlikely(lua_dump(L, writer, &state, strip) != 0)) {
...
static void trymt(lua_State *L, const char *mtname) {
  ...
  if(luai_unlikely(
       lua_type(L, 2) == LUA_TSTRING || !luaL_getmetafield(L, 2, mtname)
     )) {
    luaL_error(
      L,
...
static const char *match(MatchState *ms, const char *s, const char *p) {
  if(luai_unlikely(ms->matchdepth-- == 0)) {
...
*/
static size_t get_onecapture(
  MatchState *ms, int i, const char *s, const char *e, const char **cap
) {
  if(i >= ms->level) {
    if(luai_unlikely(i != 0)) {
    ...
    *cap           = ms->capture[i].init;
    if(luai_unlikely(capl == CAP_UNFINISHED)) {
...
static int num2straux(char *buff, int sz, lua_Number x) {
  ...
  if(x != x || x == (lua_Number)HUGE_VAL || x == -(lua_Number)HUGE_VAL) {
    return snprintf(buff, sz, LUA_NUMBER_FMT, (LUAI_UACNUMBER)x);
  } else if(x == 0) { /* can be -0... */
    /* create "0" or "-0" followed by exponent */
    return snprintf(buff, sz, LUA_NUMBER_FMT "x0p+0", (LUAI_UACNUMBER)x);
  } else {
    int e;
    ...
    n += snprintf(buff + n, sz - n, "p%+d", e); /* add exponent */
    lua_assert(n < sz);
    return n;
  }
}
...
static int lua_number2strx(
  lua_State *L, char *buff, int sz, const char *fmt, lua_Number x
) {
  ...
  } else if(luai_unlikely(fmt[SIZELENMOD] != 'a')) {
...
static void addliteral(lua_State *L, luaL_Buffer *b, int arg) {
  ...
  case LUA_TNUMBER: {
    ...
    if(!lua_isinteger(L, arg)) { /* float? */
      ...
      nb                 = snprintf(buff, MAX_ITEM, format, (LUAI_UACINT)n);
    }
    luaL_addsize(b, nb);
    break;
  }
  ...
  while(strfrmt < strfrmt_end) {
    if(*strfrmt != L_ESC) {
      ...
      intcase: {
        ...
        nb = snprintf(buff, maxitem, form, (LUAI_UACINT)n);
        break;
      }
      case 'a':
      case 'A':
...
static int getnumlimit(Header *h, const char **fmt, int df) {
  ...
  if(luai_unlikely(sz > MAXINTSIZE || sz <= 0)) {
    return luaL_error(
      h->L, "integral size (%d) out of limits [1,%d]", sz, MAXINTSIZE
    );
  }
  return sz;
}
...
static KOption getdetails(
  Header *h, size_t totalsize, const char **fmt, int *psize, int *ntoalign
) {
  ...
  if(align <= 1 || opt == Kchar) { /* need no alignment? */
    *ntoalign = 0;
    ...
    if(luai_unlikely((align & (align - 1)) != 0)) { /* not a power of 2? */
...
*/
static void
packint(luaL_Buffer *b, lua_Unsigned n, int islittle, int size, int neg) {
...
*/
static void
copywithendian(char *dest, const char *src, int size, int islittle) {
...
static int str_pack(lua_State *L) {
  ...
  while(*fmt != '\0') {
    ...
    case Kchar: { /* fixed-size string */
      ...
      luaL_argcheck(
        L, len <= (size_t)size, arg, "string longer than given size"
      );
...
static int str_packsize(lua_State *L) {
  ...
  while(*fmt != '\0') {
    ...
    KOption opt = getdetails(&h, totalsize, &fmt, &size, &ntoalign);
    luaL_argcheck(
      L, opt != Kstring && opt != Kzstr, 1, "variable-length format"
    );
...
*/
static lua_Integer
unpackint(lua_State *L, const char *str, int islittle, int size, int issigned) {
  lua_Unsigned res = 0;
  int i;
  ...
  if(size < SZINT) { /* real size smaller than lua_Integer? */
    ...
    for(i = limit; i < size; i++) {
      if(luai_unlikely((unsigned char)str[islittle ? i : size - 1 - i] != mask)) {
...
static int str_unpack(lua_State *L) {
  ...
  while(*fmt != '\0') {
    ...
    luaL_argcheck(
      L, (size_t)ntoalign + size <= ld - pos, 2, "data string too short"
    );
