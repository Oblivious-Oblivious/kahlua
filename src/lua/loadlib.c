...
*/
static void setpath(
  lua_State *L, const char *fieldname, const char *envname, const char *dft
) {
  const char *dftmark;
...
static int ll_loadlib(lua_State *L) {
  ...
  if(luai_likely(stat == 0)) { /* no errors? */
...
static const char *findfile(
  lua_State *L, const char *name, const char *pname, const char *dirsep
) {
  const char *path;
...
static void findloader(lua_State *L, const char *name) {
  ...
  if(luai_unlikely(
       lua_getfield(L, lua_upvalueindex(1), "searchers") != LUA_TTABLE
     )) {
  ...
  for(i = 1;; i++) {
    ...
    if(luai_unlikely(lua_rawgeti(L, 3, i) == LUA_TNIL)) { /* no more searchers? */
